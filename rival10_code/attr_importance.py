...
def plot_bg_ablated_accs_grouped(groups, colors):
    ...
    Does so both for models finetuned on standard images and models finetuned on FG-only images.
    '''
    d = load_cached_results(save_root='./attr_importance_dir/stats', key='acc_bg_ablated', overwrite=False)
...
if __name__ == '__main__':
    ...
    # Generates Bar plot of Accuracy on ablated BGs for both normal and fg_only models)
    plot_model_accs_bg_ablation(_ALL_MTYPES)

    # Generates attribute ablation plot (heatmap)
