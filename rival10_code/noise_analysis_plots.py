...
def hist_per_model_bg_v_fg(mtypes, linf=True, n_per_row=5, scatter=False, class_idx_to_display=[*range(10)]):
    '''
    Per model, generates 2d histograms or scatter plots for instance wise model performance.  
    '''
...
def view_extreme_cases(mtypes, eps=150, linf=True, worst=True):
    '''
    Visualizes examples where noise in background is much more damaging than noise in fg. 
    '''
...
def vit_patchsize_effect(f, ax, linf=True):
    '''
    Bar plot on effect of patchsize for ViTs (larger patchsize --> lower RFS)
    '''
...
def adv_training_effect(f, ax, linf=True):
    '''
    Generates bar plot on effect of adv training (w/ diff epsilons) on RFS
    '''
...
if __name__ == '__main__':
    ...
    # View extreme examples (where bg noise is much more damaging than fg noise)
    mtypes = ['robust_resnet50', 'simclr','deit_tiny','deit_small', 'deit_base']
    for mtype in mtypes:
        extract_extreme_cases(mtype, loader, eps=60)
    view_extreme_cases(mtypes, eps=60)

    # Generate scatter plot of average accs under fg vs bg noise for many different models
    plot_fg_bg_scatter(_ALL_MTYPES, stat='acc', linf=True)

    # RFS over different noise levels for models categorized by architecture/training procedure
    plot_noise_robs(linf=True, plot2_stat='normed_diff')

    # Smoothed histograms + scatter plots for per class analysis
    ...
    # Ablations on specific effects (patchsizes in ViTs, eps of adv training)
